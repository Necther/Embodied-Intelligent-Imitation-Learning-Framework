...
python lerobot/scripts/control_robot.py record \
    ...
    --episode-time-s 30 \
    --reset-time-s 10 \
    -p lerobot/scripts/080000/pretrained_model
```
...
def record(
    ...
    log_say("今日工作开始了，action", play_sounds)
    ...
    while True:
        ...
        log_say(f"开始录制剧集 {episode_index}", play_sounds)
        record_episode(
            dataset=dataset,
            robot=robot,
            events=events,
        ...
        if not events["stop_recording"] and (
            ...
            log_say("快重新部署环境", play_sounds)
            reset_environment(robot, events, reset_time_s)

        if events["rerecord_episode"]:
            log_say("再次录制剧集", play_sounds)
...
def replay(
    ...
    log_say("播放剧集", play_sounds, blocking=True)
...
if __name__ == "__main__":
    ...
    print("当前模型", control_mode)
    robot_path = args.robot_path
    
    robot_overrides = args.robot_overrides
    print("参数", robot_overrides)
    kwargs = vars(args)
    del kwargs["mode"]
